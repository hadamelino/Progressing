...
extension ProgressViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        if section == 0 {
            ...
            myLabel.text = "Learning Progress"
        }
        else {
            myLabel.frame = CGRect(x: 15, y: 0, width: 320, height: 20)
            myLabel.text = "High Priority Learning Objectives"
        }
...
extension ProgressViewController: HighPriorityLODelegate {
    ...
    func didTapLearning(with highPriority: LearningObjective) {
        ...
        let exposedAction = UIAlertAction(title: "Beginning", style: .default) { alert in
        ...
        let withHelpAction = UIAlertAction(title: "Progressing", style: .default) { alert in
            self.viewModel.patchLearningProgress(with: alert.title ?? "", pageID: highPriority.id)
            self.viewModel.updateLearningProgress(loToUpdate: highPriority, progress: alert.title!)
        }
        let withoutHelpAction = UIAlertAction(title: "Proficient", style: .default) { alert in
            self.viewModel.patchLearningProgress(with: alert.title ?? "", pageID: highPriority.id)
            self.viewModel.updateLearningProgress(loToUpdate: highPriority, progress: alert.title!)
        }
        let teachLessonAction = UIAlertAction(title: "Examplary", style: .default) { alert in
            self.viewModel.patchLearningProgress(with: alert.title ?? "", pageID: highPriority.id)
            self.viewModel.updateLearningProgress(loToUpdate: highPriority, progress: alert.title!)
        }
